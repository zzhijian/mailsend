...
<body>
    ...
    <div style="width: 540px; margin: auto; background: #fff; border-radius: 5px; padding: 30px; box-shadow: 0 1px 1px 1px #cdd0cf; margin-bottom: 30px;">
        ...
        <p style="font-size:14px; color:#666; margin-top:15px; line-height:22px;">Thank you for shopping with PatPat. We regret to inform you that your order <strong>{{$order_id}}</strong> has been cancelled due to the following possible reasons:</p>
        ...
        <p style="font-size:14px; color:#666; margin-top:15px; line-height:22px;">You are our esteemed and valuable customer, and we feel sorry for any inconvenience. We hope you can accept a $5 coupon as a small compensation. To apply the code, please input
        ...
        <div style="margin-top: 20px; border: 1px solid #cdd2d3; padding: 0 20px 20px;">
            <div style="border-bottom: 1px solid #f1f3f2; padding: 15px 0; color: #666; font-size: 16px; text-align: center; height:20px;">
                ...
                <span style="float:right;">Status: Canceled</span>  <!-- Canceled 状态需获取 -->
            </div>

            <ul style="padding:20px 0;">
                <?php
                $cancel_subtotal = 0;
                $cancel_total = $user_info['tax'] + $user_info['shipping_fee'];
                ?>
                @foreach($cancel_sku as $cancel_sku_item)
                    <?php
                    $cancel_sku_item = (object)$cancel_sku_item;
                    $cancel_subtotal += $cancel_sku_item->single_price;
                    $cancel_total += $cancel_sku_item->single_price;
                    ?>
            ...
            <div style="border-top: 1px solid #f1f3f2; padding-top: 20px;">
                ...
                <span style="display: block;line-height: 26px;font-size: 14px;color: #666;">Subtotal: <i style="float: right;font-style: normal;">${{$cancel_subtotal}}</i> </span>
                ...
                <div style="padding-top: 20px; color: #f1435a; font-size: 16px;">
                    Total: <i style="float:right;font-style:normal; font-size:20px;">${{$cancel_total}}</i>
                </div>
            </div>
        </div>
    </div>
